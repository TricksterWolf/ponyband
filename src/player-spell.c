...
/**
 * Stat Table (DEX/INT/WIS/CHA) -- Minimum failure rate (percentage)
 */
...
/**
 * Stat Table (DEX/INT/WIS/CHA) -- failure rate adjustment
 */
...
bool spell_cast(int spell_index, int dir)
{
	...
	if (randint0(100) < chance) {
		...
		/* Cast the spell */
		if (!effect_do(spell->effect, ident, TRUE, dir, beam, FALSE)) {
			mem_free(ident);
			return FALSE;
		}
