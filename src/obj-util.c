...
int tval_idx_to_realm_idx(byte tval) {
    int i;
    const char *tval_name = tval_find_name(tval);
        
    for (i = 0; i < REALM_MAX; i++) {
        if (!my_stricmp(tval_name, realms[i].book_noun)) {
            return i;
        }
    }
...
int realm_name_to_idx(const char *name) {
    int i;
    for (i = 0; i < REALM_MAX; i++) {
        if (!my_stricmp(name, realms[i].adjective)) {
            return i;
        }
    }
...
int spell_name_to_idx(const char *name) {
    struct magic_spell *spell = m_info;

    int i = 0;
    for (i = 0 ; i < z_info->s_max ; i++) {
        if (!my_stricmp(name, spell->name)) {
            return i;
        }
        spell = spell->next;
    }
