...
static enum parser_error parse_spell_effect(struct parser *p) {
    struct magic_spell *spell = parser_priv(p);
...
static enum parser_error parse_spell_param(struct parser *p) {
    struct magic_spell *spell = parser_priv(p);
...
struct parser *init_parse_spell(void) {
    struct parser *p = parser_new();

    m_info = mem_zalloc(sizeof (*m_info));
    parser_setpriv(p, m_info);

    parser_reg(p, "name str name", parse_spell_name);
    parser_reg(p, "effect sym eff ?sym type ?int xtra", parse_spell_effect);
    parser_reg(p, "param int p2 ?int p3", parse_spell_param);
...
static errr finish_parse_spell(struct parser *p) {
    struct magic_spell *walker;

    z_info->s_max = -1;
    walker = parser_priv(p);
    while (walker) {
        z_info->s_max++;
        walker = walker->next;
    }
...
static void cleanup_spell(void) {
    struct magic_spell *spell = m_info;
...
static struct file_parser spell_parser = {
    "spell",
    init_parse_spell,
    run_parse_spell,
    finish_parse_spell,
...
static enum parser_error parse_object_spell(struct parser *p) {
    struct object_kind *k = parser_priv(p);
    assert(k);

    const char *spell_name;
    spell_name = parser_getstr(p, "spell");

    struct magic_spell *new_spell;
    struct magic_spell *spell = m_info;
    
    int i = spell_name_to_idx(spell_name);

    if (i < 0) {
        return PARSE_ERROR_INVALID_SPELL_NAME;
    }
   
    new_spell = mem_zalloc(sizeof (*new_spell));
    new_spell->name = spell->name;
    new_spell->text = spell->text;
...
struct parser *init_parse_object(void) {
    ...
    parser_reg(p, "spell str spell", parse_object_spell);
...
/**
 * Parsing functions for p_race.txt
 */
...
struct parser *init_parse_class(void) {
    ...
    parser_reg(p, "spell sym name int level int mana int fail", parse_class_spell);
